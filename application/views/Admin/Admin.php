...
<script src="http://localhost/quiz/assetes/jquery.min.js"></script>
	...
	<script src="http://localhost/quiz/assetes/bootstrap.min.jquerys"></script>
	 ...
	 <!-- Js file Include-->
	 <script type="<?= base_url();?>assets/js/Self_test/class_test_home.js" ></script>
	</head>
			...
			<div class="row">
				...
				<div class="col-xs-10">
					<div class="tab-content">
						...
						<div>
						<form id="results_form" action="http://localhost/quiz/index.php/admin_controller/Allow" method="post">
						<table class="table">
						<tr>
						<td>Name</td>
						<td>Staus</td>
						<td>Enable Or Diasble </td>
						</tr>
					   <?php foreach ($link_en_di_able as $key) {?>
					   <tr>
                    <td><input type="hidden" name="username" value="<?php echo $key->username; ?>"> <?php echo $key->username;?></td>
					<td><?php if($key->status==0){echo "Disable Linke";}else{echo "Enable Linke";} ?></td>
					<td> <button type="Allow" class="btn btn-success">Submit Test</button></td>
					<tr>
                     <?php } ?>
						</table>
						</form>
